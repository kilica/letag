...
define('LETAG_FRAME_PERFORM_SUCCESS', 1);
define('LETAG_FRAME_PERFORM_FAIL', 2);
define('LETAG_FRAME_INIT_SUCCESS', 3);

define('LETAG_FRAME_VIEW_NONE','none');
define('LETAG_FRAME_VIEW_SUCCESS','success');
define('LETAG_FRAME_VIEW_ERROR','error');
define('LETAG_FRAME_VIEW_INDEX','index');
define('LETAG_FRAME_VIEW_INPUT','input');
define('LETAG_FRAME_VIEW_PREVIEW','preview');
define('LETAG_FRAME_VIEW_CANCEL','cancel');

/**
 * Letag_Module
**/
class Letag_Module extends Legacy_ModuleAdapter
{
	public /*** string ***/ $mActionName = null;

	public /*** Letag_AbstractAction ***/ $mAction = null;

	public /*** bool ***/ $mAdminFlag = false;

	public /*** Letag_AssetManager ***/ $mAssetManager = null;
	...
	protected /*** Enum[] ***/ $_mAllowViewNames = array(
		LETAG_FRAME_VIEW_NONE,
		LETAG_FRAME_VIEW_SUCCESS,
		LETAG_FRAME_VIEW_ERROR,
		LETAG_FRAME_VIEW_INDEX,
		LETAG_FRAME_VIEW_INPUT,
		LETAG_FRAME_VIEW_PREVIEW,
		LETAG_FRAME_VIEW_CANCEL
	);
	...
	**/
	public function startup()
	{
		...
		XCube_DelegateUtils::call('Module.letag.Global.Event.GetAssetManager',new XCube_Ref($this->mAssetManager),$this->mXoopsModule->get('dirname'));
	...
	**/
	private function _getDefaultActionName()
	{
		...
		$dataname = isset($dataname) ? $dataname : 'Tag';
	...
	**/
	public function getRenderSystemName()
	{
		...
		if(!defined('LETAG_ADMIN_RENDER_REGISTED'))
		{
			define('LETAG_ADMIN_RENDER_REGISTED',true);
			$root =& XCube_Root::getSingleton();
			$root->overrideSiteConfig(
				array(
					'RenderSystems' => array(
						'Letag_AdminRenderSystem' => 'Letag_AdminRenderSystem'
					),
					'Letag_AdminRenderSystem' => array(
						'root' => LETAG_TRUST_PATH,
						'path' => '/admin/class/LetagAdminRenderSystem.class.php',
						'class' => 'Letag_AdminRenderSystem'
					)
				)
			);
		}
	
		return 'Letag_AdminRenderSystem';
	}
	...
	private function _createAction()
	{
		...
		switch(true)
		{
			case file_exists(
				...
				break;
			case file_exists(
				$path = LETAG_TRUST_PATH . '/' . $fileName
			):
				break;
			default:
				return false;
		...
		$className = 'Letag_' . ($this->mAdminFlag ? 'Admin_' : '')
			. ucfirst($this->mActionName) . 'Action';
		if(class_exists($className))
		{
			$this->mAction = new $className();
		}
		if(!$this->mAction instanceof Letag_AbstractAction)
		{
			return false;
		}
	...
	**/
	...
	{
		/**
		 * Module.letag.Global.Event.Exception.ActionNotFound
		 * 
		 * @param	string	$dirname
		 * 
		 * @return	void
		**/
		XCube_DelegateUtils::call('Module.letag.Global.Event.Exception.ActionNotFound',$this->mAssetManager->mDirname);
		/**
	...
	**/
	...
	{
		/**
		 * Module.letag.Global.Event.Exception.Preparation
		 * 
		 * @param	string	$dirname
		 * 
		 * @return	void
		**/
		XCube_DelegateUtils::call('Module.letag.Global.Event.Exception.Preparation',$this->mAssetManager->mDirname);
		/**
	...
	**/
	...
	{
		/**
		 * Module.letag.Global.Event.Exception.Permission
		 * 
		 * @param	string	$dirname
		 * 
		 * @return	void
		**/
		XCube_DelegateUtils::call('Module.letag.Global.Event.Exception.Permission',$this->mAssetManager->mDirname);
		/**
		...
		**/
		...
		$root =& XCube_Root::getSingleton();
		$root->mController->executeRedirect(XOOPS_URL, 1, _MD_LETAG_ERROR_PERMISSION);
	}
}
